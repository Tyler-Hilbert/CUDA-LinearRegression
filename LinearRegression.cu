...
int main() {
    ...
    cudaMalloc((void**)&d_den, sizeof(float));
    err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("13CUDA error: %s\n", cudaGetErrorString(err));
    }
    cudaMemset(d_num, 0.0f, sizeof(float));
    err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("13.aCUDA error: %s\n", cudaGetErrorString(err));
    }
    cudaMemset(d_den, 0.0f, sizeof(float));
    err = cudaGetLastError();
    if (err != cudaSuccess) {
    ...
    cudaMemcpy(&numerator, d_num, sizeof(float), cudaMemcpyDeviceToHost);
    err = cudaGetLastError();
    if (err != cudaSuccess) {
        printf("15CUDA error: %s\n", cudaGetErrorString(err));
    }
    cudaMemcpy(&denominator, d_den, sizeof(float), cudaMemcpyDeviceToHost);
    err = cudaGetLastError();
    if (err != cudaSuccess) {
